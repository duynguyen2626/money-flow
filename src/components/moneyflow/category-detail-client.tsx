...
import { Category, TransactionWithDetails, Account, Person, Shop } from "@/types/moneyflow.types"
import { UnifiedTransactionTable } from "@/components/moneyflow/unified-transaction-table"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { cn } from "@/lib/utils"
import { Archive, ChevronLeft, Search, Loader2, Plus, Copy } from "lucide-react"
...
export function CategoryDetailClient({ category, transactions, allCategories, allShops, accounts, people }: CategoryDetailClientProps) {
    ...
    useEffect(() => {
        if (category.is_archived) {
            setCustomName(pathname, "Archived Category")
        } else {
            setCustomName(pathname, category.name)
        }
    }, [category.is_archived, category.name, pathname, setCustomName])
    ...
    const initialDataForNewTxn = useMemo(() => {
        // Find first shop that has this category as default
        const defaultShop = allShops.find(s => s.default_category_id === category.id)
        return {
            category_id: category.id,
            shop_id: defaultShop?.id || null,
            occurred_at: new Date(),
            type: (category.type === 'income' ? 'income' : 'expense') as any
        }
    }, [category, allShops])
                ...
                <div className="max-w-[1800px] mx-auto">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <div className="flex items-center gap-4">
                            <Link href="/categories?tab=categories">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden shadow-sm uppercase font-black text-slate-400">
                                    {category.image_url ? (
                                        <img src={category.image_url} alt="" className="w-full h-full object-contain p-2" />
                                    ) : (
                                        <span>{category.icon || category.name[0]}</span>
                                    )}
                                </div>
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                        <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase">{category.name}</h1>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest bg-slate-50 px-2 py-0.5 rounded border border-slate-100">{category.type}</span>
                                        <span className="text-[10px] font-mono text-slate-300">{category.id}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
