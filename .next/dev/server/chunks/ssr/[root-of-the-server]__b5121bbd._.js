...
function getPocketBaseAuthHeaders() {
    const token = process.env.POCKETBASE_ADMIN_TOKEN || process.env.POCKETBASE_SERVICE_TOKEN || process.env.POCKETBASE_TOKEN || process.env.PB_ADMIN_TOKEN || process.env.PB_SERVICE_TOKEN || '';
    return token ? {
        ...
        if (legacySupabaseId) {
            ...
            if (fallbackMapped.length > 0) {
                ...
                if (pbFieldBackfills.length > 0) {
                    ...
                    const results = await Promise.allSettled(uniqueBackfills.map((item)=>fetch(`${pbBaseUrl}/api/collections/pvl_txn_001/records/${item.id}`, {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json',
                                ...getPocketBaseAuthHeaders()
                            },
                            body: JSON.stringify(item.payload),
                            cache: 'no-store'
                        })));
                    const successCount = results.filter((result)=>result.status === 'fulfilled' && result.value.ok).length;
                    const failureDetails = await Promise.all(results.filter((result)=>result.status === 'fulfilled' && !result.value.ok).slice(0, 3).map(async (result)=>{
                        let bodyText = '';
                        try {
                            bodyText = await result.value.text();
                        } catch  {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b5121bbd._.js.map