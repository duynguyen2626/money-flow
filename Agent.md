...
WORKFLOW:
Branch: fix/phase-40-consolidation
...
Fix: Ensure created_at or occurred_at includes time. If occurred_at is date-only (00:00), try to fetch created_at for the Time part.
Type:
...
Limit: account.credit_limit (e.g., 50M).
Balance: account.current_balance (e.g., -5M).
...
IV. EXECUTION STEPS
Frontend: Update AccountCard math logic.
Frontend: Polish UnifiedTransactionTable columns (Notes, Time).
Frontend: Fix TransactionForm to save full datetime.
Integration: Ensure /people page refreshes on actions.
