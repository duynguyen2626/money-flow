// Created by Codex on 2025-12-27 18:04 +07
/**
 * sheetService.js
...
function syncTransactionsToSheet(sheet, transactions) {
    ...
    // Sort transactions by date ascending for a natural timeline view.
    ...
    safeRun('hideIdColumn', function () { sheet.hideColumns(1); });
    safeRun('setupShopIconFormula', function () { setupShopIconFormula(sheet); });
    safeRun('applyTableBorders', function () { applyTableBorders(sheet); });
    ...
    safeRun('applyColumnLayout', function () { applyColumnLayout(sheet); });
...
function upsertTransactionRow(sheet, txn, action) {
    ...
    safeRun('sortDataRowsByDate', function () { sortDataRowsByDate(sheet); });
    safeRun('applyCalcFormulas', function () { applyCalcFormulas(sheet); });
    safeRun('hideIdColumn', function () { sheet.hideColumns(1); });
    safeRun('setupShopIconFormula', function () { setupShopIconFormula(sheet); });
    safeRun('applyTableBorders', function () { applyTableBorders(sheet); });
...
function applyColumnLayout(sheet) {
...
function applyTableBorders(sheet) {
    ...
    if (maxRows > 1) {
        sheet.getRange(2, 1, maxRows - 1, 10)
            .setBorder(false, false, false, false, false, false);
    }
    var lastRow = getLastDataRow(sheet);
    if (lastRow >= 2) {
        sheet.getRange(2, 1, lastRow - 1, 10)
            .setBorder(true, true, true, true, true, true);
    }
}
...
function sortDataRowsByDate(sheet) {
    ...
    var shopValues = sheet.getRange(2, 15, lastRow - 1, 1).getValues();
    ...
    for (var i = 0; i < dataValues.length; i++) {
        ...
        if (!id) continue;
        var dateValue = row[2];
        var dateObj = dateValue instanceof Date ? dateValue : new Date(dateValue);
        rows.push({
    ...
    var sortedValues = rows.map(function (item) { return item.row; });
    var sortedShops = rows.map(function (item) { return [item.shop]; });
    ...
    if (sortedValues.length > 0) {
        sheet.getRange(2, 1, sortedValues.length, 8).setValues(sortedValues);
        sheet.getRange(2, 15, sortedShops.length, 1).setValues(sortedShops);
    }
}
